...
public class Main {
	...
	private static String nameOfProject = "covertChannelUsingTwoProcessesMeasuringSingleAccesses_withoutHeatmap";
		...
		for (int i = 0; i < lines.length; i++) {
			try {
				if (lines[i].length() > 14 && lines[i].substring(0, 8).equals("received")) {
					...
					if (lines[i].length() > 15 && lines[i].substring(0, 13).equals("time measured")
							&& lastWasReceived) {
				...
				// there was a problem with the format, just show and skip
				e.printStackTrace();
			}
	...
	public static void main(String[] args) {
		de.alzeha.Uni.BA.Java.DebuggingCovertChannel.TxtReader cc;
		cc = new de.alzeha.Uni.BA.Java.DebuggingCovertChannel.TxtReader(remote, nameOfProject);
